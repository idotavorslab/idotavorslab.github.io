const NeuroanatomyPage = () => {
    ...
    async function init() {
        ...
        const brains = [];
        for (let [title, url] of Object.entries(<TMap<string>>data)) {
            let brain = div({cls: 'brain'}).append(
                elem({tag: 'h1'}).text(title),
                div({cls: 'sketchfab-embed-wrapper'}).html(wrapSketch(title, url))
            );
            brains.push(brain);
        }
    ...
    function wrapSketch(title: string, url: string): string {

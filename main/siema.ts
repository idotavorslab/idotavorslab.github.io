type TConfig = { selector: string; duration: number; easing: string; perPage: number; startIndex: number; draggable: boolean; multipleDrag: boolean; threshold: number; loop: boolean; rtl: boolean; onInit: () => void; onChange: () => void; };

type TDrag = { startX: number; endX: number; startY: number; letItGo: any; preventClick: boolean; };
...
class Siema {
     ...
     */
    static mergeSettings(options: TConfig) {
        const settings = {
            selector: '.siema',
            duration: 200,
            easing: 'ease-out',
